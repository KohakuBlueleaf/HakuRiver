...
<template>
  <div>
    ...
    <el-dialog
      ...
      draggable
    >
      <el-form ref="vpsFormRef" :model="vpsForm" :rules="vpsFormRules" label-position="top" @submit.prevent="handleVpsSubmit">
        <!-- Public Key Input -->
          ...
          >
            ...
            <template #tip>
              <el-text size="small" type="info" style="margin-left: 10px"
                >Upload your public key file (e.g., ~/.ssh/id_rsa.pub)</el-text
              >
            </template>
          </el-upload>
        </el-form-item>
        ...
        <el-form-item label="Container Environment" prop="container_name">
          ...
          >
            <el-option label="[Use Host Default]" :value="null" />
            <!-- Use null for default -->
            <el-option label="[Systemd Fallback - NOT FOR VPS]" value="NULL" disabled />
            <!-- Explicitly disabled -->
            <el-option
          ...
          <el-text size="small" type="info"
            >Select the Docker environment tarball to use. VPS tasks *require* a Docker container environment. Selecting Host
            Default uses the tarball configured on the Host.</el-text
          >
        </el-form-item>
        ...
        <el-form-item v-if="!vpsForm.gpuFeatureEnabled" label="Target Node / NUMA Node" prop="selectedTarget">
          ...
          >
            <el-option label="[Auto Select]" :value="null" />
            <!-- Use null for auto -->
            <el-option v-for="item in targetOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
          <el-text size="small" type="info"
            >Select a single node or node:numa_id, or leave blank for auto-selection based on required cores.</el-text
          >
        </el-form-item>
...
const parseMemoryToBytes = (memStr) => {
  ...
  if (!match) {
    throw new Error(`Invalid memory format: '${memStr}'. Use suffix K, M, or G (e.g., 512M, 4G).`);
  }
...
const vpsFormRules = computed(() => {
  const rules = {
    ...
    required_cores: [
      { required: false, type: 'number', min: 0, message: 'Cores must be a non-negative number', trigger: 'blur' },
    ],
...
const handleKill = (taskId) => {
  ...
  ElMessageBox.confirm(
    `Are you sure you want to kill VPS task ${taskId}? This will stop the container and cannot be undone.`,
    'Confirm Kill VPS',
    {
...
const handlePauseResume = (taskId, status) => {
  ...
  ElMessageBox.confirm(
    `Are you sure you want to ${action} VPS task ${taskId}?`,
    `Confirm ${action.charAt(0).toUpperCase() + action.slice(1)} VPS`,
    {
...
watch(
  ...
  { deep: true }
); // Watch deeply into selectedGpus object
</script>
