...
- **RAM**: 4GB+
- **Disk**:
  - 10GB+ for the Runner software and Docker
...
Configure Docker storage driver and data root:
...
**Important considerations for `data-root`**:
- Choose a location with sufficient disk space for container images
- For compute nodes with multiple drives, consider using a separate disk for Docker data
- SSD storage will provide better container start times
