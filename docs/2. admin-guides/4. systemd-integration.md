...
systemd allows fine-grained resource control for services:

### CPU Control

Limit CPU usage for a service:

```bash
# Set CPU quota (percentage across all cores)
sudo systemctl set-property hakuriver-runner.service CPUQuota=400%

# Limit to specific CPU cores
sudo systemctl set-property hakuriver-runner.service AllowedCPUs=0-3
```
...
```bash
# Set memory limit
sudo systemctl set-property hakuriver-runner.service MemoryMax=8G

# Set memory and swap limit
sudo systemctl set-property hakuriver-runner.service MemoryMax=8G
sudo systemctl set-property hakuriver-runner.service MemorySwapMax=10G
```
...
```bash
# Set I/O weight (default is 100)
sudo systemctl set-property hakuriver-host.service IOWeight=200
```

### Viewing Current Settings

```bash
systemctl show hakuriver-runner.service -p CPUQuota,MemoryMax
```
...
### Host Requiring Database

If your Host uses an external database:
...
```
Task execution failed: Resource temporarily unavailable
```

Solution:
- Increase resource limits in the service file
- Check system resource availability
- Monitor with `top` or `htop` during service startup
