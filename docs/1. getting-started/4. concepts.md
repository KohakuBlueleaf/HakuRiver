...
In addition to SSH proxy access, KohakuRiver provides WebSocket-based terminal access:
- **Purpose:** Access running tasks directly through the web dashboard or CLI without requiring SSH client setup.
- **Usage:** Use `kohakuriver connect task <task_id>` or `kohakuriver connect docker <container_name>` from the CLI, or click the terminal button in the web dashboard.
