...
| `kohakuriver vps` | VPS management (create, list, stop) |
| `kohakuriver node` | Node management (list) |
| `kohakuriver docker` | Docker/container management |
| `kohakuriver ssh` | SSH to VPS via proxy |
| `kohakuriver connect` | WebSocket terminal to tasks |
...
| **VPS Tasks** | Persistent containers with SSH access |
...
| Feature | Description |
|---------|-------------|
| **SSH Proxy** | Connect to VPS through Host relay |
