...
<template>
  <div class="space-y-6">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <el-tooltip content="Copy VPS ID" placement="top">
                  <button
                    @click="navigator.clipboard.writeText(vps.task_id); notify.success('VPS ID copied')"
    ...
    <el-dialog v-model="createDialogVisible" title="Create VPS" width="600px" destroy-on-close>
      <el-form :model="createForm" label-position="top">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <el-form-item label="Memory">
            <el-input-number v-model="createForm.required_memory_bytes" :min="0" placeholder="Bytes" class="w-full" />
          </el-form-item>
        </div>

        <el-form-item label="Container Environment">
          <el-select v-model="createForm.container_name" placeholder="Select container" clearable class="w-full">
            <el-option
        ...
        <el-form-item v-if="!createForm.gpuFeatureEnabled" label="Target Node">
          <el-select v-model="createForm.target_hostname" placeholder="Auto-select" clearable class="w-full">
            <el-option
        ...
        <el-form-item v-else label="Select Target GPUs">
          <div class="gpu-selection-container">
            <el-empty v-if="nodesWithGpus.length === 0" description="No online nodes with GPUs" :image-size="60" />
            <el-collapse v-else v-model="expandedGpuNodes">
              <el-collapse-item v-for="node in nodesWithGpus" :key="node.hostname" :name="node.hostname">
                <template #title>
      ...
      <template #footer>
        <div class="flex flex-col sm:flex-row gap-2 sm:justify-end">
          <el-button @click="createDialogVisible = false" class="w-full sm:w-auto">Cancel</el-button>
          <el-button type="primary" :loading="vpsStore.creating" @click="handleCreate" class="w-full sm:w-auto">
            Create
          </el-button>
        </div>
      </template>
    ...
    <el-dialog v-model="generatedKeyResult" title="SSH Key Generated" width="600px" :close-on-click-modal="false">
      <div class="space-y-4">
        <el-alert type="warning" :closable="false"> Save this private key now. It will not be shown again. </el-alert>
      ...
      <template #footer>
        <div class="flex flex-col sm:flex-row gap-2 sm:justify-end">
          <el-button @click="downloadPrivateKey" class="w-full sm:w-auto">
            <span class="i-carbon-download mr-1"></span> Download Key
          </el-button>
          <el-button type="primary" @click="generatedKeyResult = null" class="w-full sm:w-auto">Done</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
