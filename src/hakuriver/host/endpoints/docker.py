...
from hakuriver.docker.naming import ENV_PREFIX, is_hakuriver_container
...
async def list_host_containers():
    ...
    Returns only containers with the hakuriver-env- prefix.
...
async def create_host_container(request: CreateContainerRequest):
    ...
    The container name will be prefixed with 'hakuriver-env-' to identify it
...
def _do_migrate_container(old_name: str, new_name: str) -> dict:
    ...
    Args:
        ...
        new_name: New container name with hakuriver-env- prefix
    ...
    image_repo = f"hakuriver/{old_name}"
    image_tag = "base"
...
async def migrate_container(old_name: str):
    """Migrate a legacy container to the new hakuriver-env- naming convention.

    Renames container from '{old_name}' to 'hakuriver-env-{old_name}'.
