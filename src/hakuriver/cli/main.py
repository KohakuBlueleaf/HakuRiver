"""
HakuRiver unified CLI entry point.

Usage:
    hakuriver [OPTIONS] COMMAND [ARGS]...

Commands:
    task      Task management
    vps       VPS management
    node      Node management
    docker    Docker/container management
    ssh       SSH commands
    terminal  Terminal access
    config    Configuration
"""

from typing import Annotated

import typer

from hakuriver.cli import config as cli_config
from hakuriver.cli.commands import (
    config_cmd,
    connect,
    docker,
    node,
    ssh,
    task,
    terminal,
    vps,
)
from hakuriver.cli.output import console

app = typer.Typer(
    name="hakuriver",
    help="HakuRiver Cluster Management CLI",
    no_args_is_help=True,
    rich_markup_mode="rich",
)

# Register command groups
app.add_typer(task.app, name="task", help="Task management")
app.add_typer(vps.app, name="vps", help="VPS management")
app.add_typer(node.app, name="node", help="Node management")
app.add_typer(docker.app, name="docker", help="Docker/container management")
app.add_typer(ssh.app, name="ssh", help="SSH commands")
app.add_typer(terminal.app, name="terminal", help="Terminal access (TUI)")
app.add_typer(connect.app, name="connect", help="Connect to container terminal")
app.add_typer(config_cmd.app, name="config", help="Configuration")


@app.callback()
def main(
    host: Annotated[
        str | None,
        typer.Option("--host", "-H", help="Host address", envvar="HAKURIVER_HOST"),
    ] = None,
    port: Annotated[
        int | None,
        typer.Option("--port", "-P", help="Host port", envvar="HAKURIVER_PORT"),
    ] = None,
    output_format: Annotated[
        str,
        typer.Option("--format", "-f", help="Output format: table|json|yaml"),
    ] = "table",
):
    """
    HakuRiver Cluster Management CLI.

    Manage tasks, VPS instances, nodes, and Docker containers.
    """
    if host:
        cli_config.HOST_ADDRESS = host
    if port:
        cli_config.HOST_PORT = port
    cli_config.OUTPUT_FORMAT = output_format


@app.command("version")
def version():
    """Show version information."""
    try:
        from hakuriver import __version__

        console.print(f"HakuRiver CLI v{__version__}")
    except ImportError:
        console.print("HakuRiver CLI (version unknown)")


@app.command("status")
def quick_status():
    """Show quick cluster status overview."""
    from hakuriver.cli import client
    from hakuriver.cli.formatters.node import format_cluster_summary
    from hakuriver.cli.formatters.task import format_task_list_compact
    from hakuriver.cli.output import print_error

    try:
        # Get nodes
        nodes = client.get_nodes()

        if nodes:
            summary = format_cluster_summary(nodes)
            console.print(summary)
            console.print()

        # Get running tasks
        tasks = client.get_tasks(status="running", limit=10)
        if tasks:
            console.print("[bold]Running Tasks:[/bold]")
            table = format_task_list_compact(tasks)
            console.print(table)
        else:
            console.print("[dim]No running tasks.[/dim]")

    except client.APIError as e:
        print_error(str(e))
        raise typer.Exit(1)


def run():
    """Entry point for the CLI."""
    app()


if __name__ == "__main__":
    run()
