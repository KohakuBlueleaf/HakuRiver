...
from hakuriver.docker.naming import image_tag
...
def create_container_tar(
    ...
    Args:
        ...
        hakuriver_container_name: HakuRiver container name.
        container_tar_dir: Path to store the tarball.

    Returns:
        Path to the created tarball, or None if failed.
    """
    tag = image_tag(hakuriver_container_name)
    timestamp = int(time.time())
    tarball_filename = f"{hakuriver_container_name.lower()}-{timestamp}.tar"
    tarball_path = os.path.join(container_tar_dir, tarball_filename)

    logger.info(
        f"Creating tarball for '{hakuriver_container_name}' "
